...
// Resolve proxy directory: CORDELIA_PROXY_DIR env, config.toml, or default
function resolveProxyDir() {
  if (process.env.CORDELIA_PROXY_DIR) return process.env.CORDELIA_PROXY_DIR;
  try {
    const configPath = join(homedir(), '.cordelia', 'config.toml');
    const config = readFileSync(configPath, 'utf-8');
    const match = config.match(/proxy_dir\s*=\s*"([^"]+)"/);
    if (match) {
      const raw = match[1];
      return raw.startsWith('~') ? join(homedir(), raw.slice(1)) : raw;
    }
  } catch { /* fall through */ }
  return join(homedir(), '.cordelia', 'proxy');
}
const PROJECT_ROOT = resolveProxyDir();
