...
// Import hooks lib from SDK (sibling directory)
const { getMemoryRoot, getEncryptionKey, getProxyDir } = await import(join(__dirname, '..', 'hooks', 'lib.mjs'));

// Import compiled modules from proxy dist/
const proxyDir = await getProxyDir();
const distDir = join(proxyDir, 'dist');
