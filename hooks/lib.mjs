...
// --- Proxy Directory Resolution ---

/**
 * Resolve the cordelia-proxy directory (where dist/http-server.js lives).
 * Priority:
 *   1. CORDELIA_PROXY_DIR environment variable
 *   2. config.toml [paths] proxy_dir
 *   3. Default: ~/.cordelia/proxy
 *
 * Returns absolute path. Expands ~ to homedir.
 */
export async function getProxyDir() {
  // 1. Environment variable (highest priority)
  if (process.env.CORDELIA_PROXY_DIR) {
    const raw = process.env.CORDELIA_PROXY_DIR;
    return raw.startsWith('~') ? path.join(os.homedir(), raw.slice(1)) : raw;
  }

  // 2. config.toml
  const config = await loadConfig();
  if (config?.paths?.proxy_dir) {
    const raw = config.paths.proxy_dir;
    return raw.startsWith('~') ? path.join(os.homedir(), raw.slice(1)) : raw;
  }

  // 3. Default
  return path.join(os.homedir(), '.cordelia', 'proxy');
}
